...
function shape(sp,name){
  ...
  const AA=sp.answers();
  chk(AA.some(a=>a.k===sp.correct),name+': správná odpověď není mezi možnostmi');
  chk(new Set(AA.map(a=>a.k)).size===AA.length,name+': duplicitní možnosti');
  ...
  ['cs','en'].forEach(lg=>{
    L.setLangTest(lg);
    const w=sp.why(), st=sp.story(), q=sp.qline?sp.qline():'x';
    const labels=sp.answers().map(a=>a.l).join(' ');
    chk(typeof w==='string'&&w.length>30,name+' ['+lg+']: chybí vysvětlení');
    chk(!/undefined|NaN|\[object/.test(w),name+' ['+lg+']: vysvětlení obsahuje '+(w.match(/undefined|NaN|\[object \w+/)||[''])[0]);
    chk(!/undefined|NaN|\[object/.test(st),name+' ['+lg+']: příběh obsahuje undefined/NaN');
    chk(!/undefined|NaN/.test(q),name+' ['+lg+']: otázka obsahuje undefined');
    chk(!/undefined|NaN/.test(labels),name+' ['+lg+']: tlačítka obsahují undefined');
    if(sp.freqs) chk(!/undefined|NaN/.test(sp.freqs().map(f=>f.k+f.v).join('')),name+' ['+lg+']: frekvence rozbité');
  });
  L.setLangTest('cs');
